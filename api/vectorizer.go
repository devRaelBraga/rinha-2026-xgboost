...
func Vectorize(req *FraudRequest, norm *Normalization, mccRisk map[string]float64) [14]float32 {
	...
	// 0: amount
	v[0] = float32(clamp(req.Transaction.Amount / norm.MaxAmount))

	// 1: installments
	v[1] = float32(clamp(float64(req.Transaction.Installments) / norm.MaxInstallments))

	// 2: amount_vs_avg
	v[2] = float32(clamp((req.Transaction.Amount / req.Customer.AvgAmount) / norm.AmountVsAvgRatio))
	...
	if req.LastTx == nil {
		...
		v[5] = float32(clamp(minutes / norm.MaxMinutes))
	}
	...
	if req.LastTx == nil {
		...
		v[6] = float32(clamp(req.LastTx.KmFromCurrent / norm.MaxKm))
	}

	// 7: km_from_home
	v[7] = float32(clamp(req.Terminal.KmFromHome / norm.MaxKm))

	// 8: tx_count_24h
	v[8] = float32(clamp(float64(req.Customer.TxCount24h) / norm.MaxTxCount24h))
	...
	v[13] = float32(clamp(req.Merchant.AvgAmount / norm.MaxMerchantAvgAmount))
